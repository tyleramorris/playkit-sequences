...
// src/start-sequence.server.ts
var start_sequence_server_exports = {};
__export(start_sequence_server_exports, {
  default: () => startSequence
});
async function startSequence(payload) {
...
var SEQUENCE_API_BASE, StartSequenceResponseSchema;
var init_start_sequence_server = __esm({
  "src/start-sequence.server.ts"() {
    "use strict";
    init_esm();
    SEQUENCE_API_BASE = "https://playkit-sequences-production.up.railway.app";
    StartSequenceResponseSchema = external_exports.object({
...
// src/get-daily-forecast.server.ts
var get_daily_forecast_server_exports = {};
__export(get_daily_forecast_server_exports, {
  TemperatureUnitSchema: () => TemperatureUnitSchema,
  default: () => getDailyForecast
});
...
async function getDailyForecast(latitude, longitude, temperature_unit) {
  try {
...
var TemperatureUnitSchema, DailyUnitsSchema, DailyForecastSchema, DailyForecastResponseSchema;
var init_get_daily_forecast_server = __esm({
  "src/get-daily-forecast.server.ts"() {
    "use strict";
    init_esm();
    TemperatureUnitSchema = external_exports.enum(["celsius", "fahrenheit"]);
    DailyUnitsSchema = external_exports.object({
      time: external_exports.string(),
      weather_code: external_exports.string(),
      precipitation_sum: external_exports.string(),
      precipitation_probability_max: external_exports.string(),
      temperature_2m_max: external_exports.string(),
      temperature_2m_min: external_exports.string(),
      wind_speed_10m_max: external_exports.string()
    });
    DailyForecastSchema = external_exports.object({
      time: external_exports.array(external_exports.number()),
      weather_code: external_exports.array(external_exports.number()),
      precipitation_sum: external_exports.array(external_exports.number()),
      precipitation_probability_max: external_exports.array(external_exports.number()),
      temperature_2m_max: external_exports.array(external_exports.number()),
      temperature_2m_min: external_exports.array(external_exports.number()),
      wind_speed_10m_max: external_exports.array(external_exports.number())
    });
    DailyForecastResponseSchema = external_exports.object({
...
// src/fetch-email-templates.server.ts
var fetch_email_templates_server_exports = {};
__export(fetch_email_templates_server_exports, {
  default: () => fetchEmailTemplates
});
async function fetchEmailTemplates() {
  const url = `${SEQUENCE_API_BASE2}/templates`;
  ...
  if (!response.ok) {
    const rawBody2 = await response.text().catch(() => null);
...
var SEQUENCE_API_BASE2, EmailTemplateSchema, EmailTemplatesResponseSchema;
var init_fetch_email_templates_server = __esm({
  "src/fetch-email-templates.server.ts"() {
    "use strict";
    init_esm();
    SEQUENCE_API_BASE2 = "https://playkit-sequences-production.up.railway.app";
    ...
    EmailTemplatesResponseSchema = external_exports.array(EmailTemplateSchema);
  }
});

// global-externals:attio/server
var require_server = __commonJS({
  "global-externals:attio/server"(exports, module) {
    module.exports = ATTIO_SERVER_APP_SDK;
  }
});

// src/fetch-company-data.server.ts
var fetch_company_data_server_exports = {};
__export(fetch_company_data_server_exports, {
  default: () => fetchCompanyData
});
async function fetchCompanyData(companyRecordId) {
  const tokenAvailable = typeof import_server.ATTIO_API_TOKEN === "string" && import_server.ATTIO_API_TOKEN.length > 0;
  console.log(
    `[Attio API] Fetching company ${companyRecordId}, token available: ${tokenAvailable}, token length: ${import_server.ATTIO_API_TOKEN?.length ?? 0}`
  );
  if (!tokenAvailable) {
  ...
  const people = peopleData.data.map((person) => {
...
var init_fetch_company_data_server = __esm({
  "src/fetch-company-data.server.ts"() {
    ...
    attioHeaders = () => ({
      Authorization: `Bearer ${import_server.ATTIO_API_TOKEN}`,
      "Content-Type": "application/json",
      Accept: "application/json"
    });
    NameValueSchema = external_exports.object({
      first_name: external_exports.string().nullable().optional(),
      last_name: external_exports.string().nullable().optional(),
      full_name: external_exports.string().nullable().optional()
    });
    EmailValueSchema = external_exports.object({
      email_address: external_exports.string()
    });
    RecordValueSchema = external_exports.object({
      target_record_id: external_exports.string(),
      target_object: external_exports.string()
    });
    PersonRecordSchema = external_exports.object({
      id: external_exports.object({
        record_id: external_exports.string()
      }),
      values: external_exports.object({
        name: external_exports.array(NameValueSchema).optional(),
        email_addresses: external_exports.array(EmailValueSchema).optional()
      })
    });
    PeopleQueryResponseSchema = external_exports.object({
      data: external_exports.array(PersonRecordSchema)
    });
    ...
    CompanyRecordResponseSchema = external_exports.object({
...
var eventModules = /* @__PURE__ */ new Map();
var workflowModules = /* @__PURE__ */ new Map();
modules.set("start-sequence.server", () => Promise.resolve().then(() => (init_start_sequence_server(), start_sequence_server_exports)));
modules.set("get-daily-forecast.server", () => Promise.resolve().then(() => (init_get_daily_forecast_server(), get_daily_forecast_server_exports)));
modules.set("fetch-email-templates.server", () => Promise.resolve().then(() => (init_fetch_email_templates_server(), fetch_email_templates_server_exports)));
modules.set("fetch-company-data.server", () => Promise.resolve().then(() => (init_fetch_company_data_server(), fetch_company_data_server_exports)));
var stdin_default;
...
function main() {
  ...
  stdin_workflow_block_handlers_default = async function(blockId, handler, args) {
    ...
    const handlerData = modules2[handler];
    if (!handlerData) {
